...
// HTML&メールテンプレート設定
$temp_html = array(
	'form'			=> 'form.html' // 入力フォーム用
,	'confirm'		=> 'confirm.html' // 確認画面用
,	'completion'		=> 'completion.html' // 送信完了画面
...
function printSubmit() {
	if($_SESSION['SFM']['InputErr']) {
		// エラーがある場合のHTML出力
		$submit = <<< EOD
<input class="btn_back" type="button" value="戻る" onclick="history.back()">
EOD;
	} else {
		// 項目が正しい場合のHTML出力
		$submit = <<< EOD
<div class="clr"></div>
<p>入力内容が正しければ、送信ボタンを押して下さい。<br></p>
<div class="clr"></div>
...
default: // 入力フォーム表示
	...
	$temp_html['form'] = 'form.php'; // form.html → form.php に変更
...
class sfmClass
{
	...
	function valDataHtml($val) {
		...
		$val = nl2br($val);
		$val = (preg_match('/__Error_.+__/', $val)) ? "<span class=\"must\">{$temp_err[$val]}</span>" : $val;
		$val = ($val != '') ? $val : '&nbsp;';
