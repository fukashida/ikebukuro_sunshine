...
//index.php
	if ($_SERVER['SCRIPT_NAME'] == $dir . "/index.php") {
		$input = isset($_SESSION['input']) ? $_SESSION['input'] : array();
		$msg   = isset($_SESSION['msg'])   ? $_SESSION['msg']   : array();
		unset($_SESSION['input']);
		unset($_SESSION['msg']);

		// ▼ ここでキー存在確認しておく
		$c1 = isset($input['c1']) ? $input['c1'] : '';
		$treatment = isset($input['treatment']) ? $input['treatment'] : array();

		//メニュー
		$rad_c1 = get_radio($mst_arr_c1, 'c1', '', $c1);
		$chk_treatment = get_checkbox2($mst_treatment, "treatment", true, $treatment);
	}
...
.contactTbl table td input[type=radio] { display: none; }
.contactTbl table { border: none; }
.contactTbl table td { border: none; }
.contactTbl table tr { border: none; }
}
</style>
...
<section class="secBox">
		...
		<form name="mail_send" method="post" action="./check.php" >
			<h3 class="baseTitle">お問い合わせフォーム</h3>
			<div class="baseTbl01 contactTbl nowrap mb20">
				<div class="wrap">
					<table>
						<tr>
							<th width="150">お名前<span class="att">※</span></th>
							<td>
								<?php if (!empty($msg['name'])): ?>
									<p class="att">▼お名前を入力して下さい。</p>
								<?php endif; ?>
								<?php $name = isset($input['name']) ? $input['name'] : ''; ?>
								<input type="text" name="name" value="<?php echo htmlspecialchars($name, ENT_QUOTES, 'UTF-8'); ?>" class="formM">
							</td>
						</tr>
						<tr>
							<th>ふりがな<span class="att">※</span></th>
							<td>
								<?php if (isset($msg['kname']) && $msg['kname']): ?>
									<p class="att">▼ふりがなを入力して下さい。</p>
								<?php endif; ?>
								<?php $kname = isset($input['kname']) ? $input['kname'] : ''; ?>
								<input type="text" name="kname" value="<?php echo htmlspecialchars($kname, ENT_QUOTES, 'UTF-8'); ?>" class="formM">
							</td>
						</tr>
						<tr>
							<th>性別<span class="att">※</span></th>
							<td>
								<?php if (isset($msg['c1']) && $msg['c1']): ?>
									<p class="att">▼性別を選択して下さい。</p>
								<?php endif; ?>
								<?php echo $rad_c1; ?>
							</td>
						</tr>
						<tr>
							<th>年齢<span class="att">※</span></th>
							<td>
								<?php if (isset($msg['old']) && $msg['old']) { ?>
									<p class="att">▼年齢を入力して下さい。</p>
								<?php } ?>
								<?php $old = isset($input['old']) ? $input['old'] : ''; ?>
								<input type="text" name="old" value="<?php echo htmlspecialchars($old, ENT_QUOTES, 'UTF-8'); ?>" class="formSS">歳
							</td>
						</tr>
						<tr>
							<th>電話番号<span class="att">※</span></th>
							<td>
								<?php if (isset($msg['tel']) && $msg['tel']) { ?>
									<p class="att">▼電話番号を入力して下さい。</p>
								<?php } ?>
								<?php $tel = isset($input['tel']) ? $input['tel'] : ''; ?>
								<input type="text" name="tel" value="<?php echo htmlspecialchars($tel, ENT_QUOTES, 'UTF-8'); ?>" class="formM"><br>
								※お問い合わせメールに対するご返信は<br>電話にてさせていただきます。
							</td>
							<tr>
								<th>メールアドレス<span class="att">※</span></th>
								<td>
									<?php if (isset($msg['mail']) && $msg['mail']) { ?>
										<p class="att">▼<?php echo htmlspecialchars($msg['mail'], ENT_QUOTES, 'UTF-8'); ?></p>
									<?php } ?>
									<?php $mail = isset($input['mail']) ? $input['mail'] : ''; ?>
									<input type="text" name="mail" value="<?php echo htmlspecialchars($mail, ENT_QUOTES, 'UTF-8'); ?>" class="formL">
								</td>
							</tr>
							<tr>
								<th>メールアドレス（確認用）<span class="att">※</span></th>
								<td>
									<?php $mail_conf = isset($input['mail_conf']) ? $input['mail_conf'] : ''; ?>
									<input type="text" name="mail_conf" value="<?php echo htmlspecialchars($mail_conf, ENT_QUOTES, 'UTF-8'); ?>" class="formL">
								</td>
							</tr>
							<tr>
								<th>ご関心のある治療項目に<br>チェックしてください。 <br>複数の選択も可能です。</th>
								<td>
									<?php echo $chk_treatment; ?>
									<?php $other = isset($input['other']) ? $input['other'] : ''; ?>
									その他：<input type="text" name="other" value="<?php echo htmlspecialchars($other, ENT_QUOTES, 'UTF-8'); ?>" class="formM">
								</td>
							</tr>
							<tr>
								<th>ご意見・ご質問 ご相談内容<span class="att">※</span></th>
								<td>
									<?php if (isset($msg['comment']) && $msg['comment']) { ?>
										<p class="att">▼ご意見・ご質問 ご相談内容を入力してください。</p>
									<?php } ?>
									<?php $comment = isset($input['comment']) ? $input['comment'] : ''; ?>
									<textarea name="comment" class="txtArea"><?php echo htmlspecialchars($comment, ENT_QUOTES, 'UTF-8'); ?></textarea>
								</td>
							</tr>
					</table>
				</div>
			...
			<div class="btnarea">
				<input name="set" type="submit" value="確認画面へ">
			</div>
		</form>
</section>
	</main>
