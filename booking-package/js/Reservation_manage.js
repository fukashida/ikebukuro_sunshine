...
document.addEventListener("DOMContentLoaded", function () {
  window.addEventListener("load", function () {
    var booking_pacage_booked_customers = document.getElementById(
      "booking_pacage_booked_customers"
    );
    if (
      schedule_data != null &&
      booking_package_dictionary != null &&
      booking_pacage_booked_customers != null
    ) {
      booking_manage = new Booking_manage(
        schedule_data,
...
var changeStatusForDashboard = function (
  button,
  key,
  cancellationToken,
  accountKey,
...
function Booking_manage(schedule_data, booking_package_dictionary, webApp) {
  ...
  this._currency_info = {
    ...
    info: this._currencies[this._currency],
  };
  this._i18n = new I18n(schedule_data.locale);
  this._i18n.setDictionary(booking_package_dictionary);
  this._format = new FORMAT_COST(
    this._i18n,
    this._debug,
  ...
  this._hotel = null;
  this._servicesControl = new Booking_App_ObjectsControl(
    schedule_data,
    booking_package_dictionary
  );
  ...
  if (schedule_data.guestForDayOfTheWeekRates != null) {
    this._guestForDayOfTheWeekRates = parseInt(
      schedule_data.guestForDayOfTheWeekRates
    );
  }
  ...
  if (
    document.getElementsByTagName("title") != null &&
  ...
  if (document.getElementById("booking_package_databaseUpdateErrors") != null) {
    object._console.log(
      document.getElementById("booking_package_databaseUpdateErrors")
    );
    var databaseUpdateErrors = new Booking_Package_DatabaseUpdateErrors(
      document.getElementById("booking_package_databaseUpdateErrors")
    );
  }

  if (schedule_data.bookingBool == 1) {
    this._hotel = new Booking_Package_Hotel(
      this._currency,
      weekName,
      this._dateFormat,
      this._positionOfWeek,
      this._positionTimeDate,
      this._startOfWeek,
      this._numberFormatter,
      this._currency_info,
      booking_package_dictionary,
    ...
    this._blockPanel = document.getElementById("blockPanel");
    this._contentPanel = document.getElementById(
  ...
  this.start = function () {
    ...
    object._changeDisplayFormatBookedCustomersForDay = object.getCookie(
      object._prefix + "changeDisplayFormatBookedCustomersForDay"
    );
    if (object._changeDisplayFormatBookedCustomersForDay === null) {
      object._changeDisplayFormatBookedCustomersForDay = "list";
      object.setCookie(
        object._prefix + "changeDisplayFormatBookedCustomersForDay",
        object._changeDisplayFormatBookedCustomersForDay,
        365
      );
    }

    object._changeDisplayFormatBookedCustomersForHotel = object.getCookie(
      object._prefix + "changeDisplayFormatBookedCustomersForHotel"
    );
    if (object._changeDisplayFormatBookedCustomersForHotel === null) {
      object._changeDisplayFormatBookedCustomersForHotel = "table";
      object.setCookie(
        object._prefix + "changeDisplayFormatBookedCustomersForHotel",
        object._changeDisplayFormatBookedCustomersForHotel,
        365
      );
    }
    ...
    try {
      ...
      if (dashboardRequest.status == 1) {
        this.getReservationData(
          parseInt(dashboardRequest.month),
          parseInt(dashboardRequest.day),
          parseInt(dashboardRequest.year),
          accountKey,
          1
        );
      } else {
        this.getReservationData(
          parseInt(this._schedule_data.month),
          parseInt(this._schedule_data.day),
          parseInt(this._schedule_data.year),
          accountKey,
          1
        );
      }
  ...
  this.lookingForVisitorServices = function (target) {
    ...
    for (var key in services) {
      if (
        parseInt(services[key].key) == parseInt(target.key) &&
        parseInt(services[key].selected) == 1
      ) {
        return true;
      }
    }
  ...
  this.getReservationData = function (
    ...
    new Booking_App_XMLHttp(
      ...
      function (calendarData) {
        ...
        object._preparationTime = parseInt(
          calendarData.account.preparationTime
        );
        object._positionPreparationTime =
          calendarData.account.positionPreparationTime;
        ...
        object._servicesControl.setNationalHoliday(
          calendarData.nationalHoliday.calendar
        );
        ...
        // ▼ 電話番号整形処理（イベント委任）
        if (!window.__bookingTelHandlerRegistered) {
            document.body.addEventListener('input', function (e) {
                if (e.target && e.target.id === 'input_tel') {
                    let value = e.target.value.replace(/[^\d]/g, '');

                    // 携帯番号（090,080,070 + 8桁）
                    if (/^0[789]0\d{8}$/.test(value)) {
                    ...
                    // 固定電話
                        ...
                        if (value.startsWith('048')) {
                            ...
                            const area5 = value.substring(0, 5);
                            
                            // 5桁市外局番
                            if (fiveDigitAreas.includes(area5)) {
                                value = value.replace(/(\d{5})(\d{1})(\d{4})/, '$1-$2-$3');
                            }
                            // 4桁市外局番
                            else if (/^0\d{3}\d{6}$/.test(value)) {
                                value = value.replace(/(\d{4})(\d{2})(\d{4})/, '$1-$2-$3');
                            }
                            // 3桁市外局番
                            else if (/^0\d{2}\d{7}$/.test(value)) {
                                value = value.replace(/(\d{3})(\d{3})(\d{4})/, '$1-$2-$3');
                            }
                            // 2桁市外局番
                            else if (/^0\d{1}\d{8}$/.test(value)) {
            ...
            window.__bookingTelHandlerRegistered = true;
        }

        // ▼ callbackが存在すれば実行、それ以外はcreateCalendarを実行
  ...
  this.selectCalendarAccount = function (
    calendarData,
    month,
    day,
    year,
    selectCalendarAccountPanel,
    calendarAccountList,
    accountKey
  ) {
    var object = this;
  ...
  this.createCalendar = function (calendarData, month, day, year, accountKey) {
    ...
    object._console.log(
      "schedule_data.calendar_account = " +
        object._schedule_data.calendar_account
    );
    ...
    object.selectCalendarAccount(
      calendarData,
      month,
      day,
      year,
      selectCalendarAccountPanel,
      object._schedule_data.calendarAccountList,
      accountKey
    );
    ...
    var weekName = [
      object._i18n.get("Sun"),
      object._i18n.get("Mon"),
      object._i18n.get("Tue"),
      object._i18n.get("Wed"),
      object._i18n.get("Thu"),
      object._i18n.get("Fri"),
      object._i18n.get("Sat"),
    ];
    object._calendar = new Booking_App_Calendar(
      weekName,
      object._dateFormat,
      object._positionOfWeek,
      object._positionTimeDate,
      object._startOfWeek,
      object._i18n,
      object._debug
    );
      ...
      function (callback) {
        ...
        if (calendarData.reservation[callback.key] != null) {
          ...
          for (var key in calendarData.reservation[callback.key]) {
            ...
            if (
              parseInt(
                calendarData.reservation[callback.key][key].scheduleUnixTime
              ) >= calendarData.date.firstMonth &&
              parseInt(
                calendarData.reservation[callback.key][key].scheduleUnixTime
              ) <= calendarData.date.endMonth
            ) {
              downloadBool = true;
            }

            if (
              calendarData.reservation[callback.key][key].status == "approved"
            ) {
              approved++;
            } else if (
              calendarData.reservation[callback.key][key].status == "pending"
            ) {
              pending++;
            } else if (
              calendarData.reservation[callback.key][key].status == "canceled"
            ) {
              canceled++;
            }
          }
        ...
        if (
          (calendarData.schedule[callback.key] != null &&
            calendarData.schedule[callback.key].length != 0) ||
          total != 0
        ) {
          ...
          dayPanel.onclick = function () {
            ...
            document
              .getElementById("reservation_users")
              .setAttribute("class", "media_menu_item active");
            document
              .getElementById("add_reservation")
              .setAttribute("class", "media_menu_item");
    ...
    if (calendarAccount.timezone != "none") {
      timezonePanel.textContent =
        object._i18n.get("Timezone") + ": " + calendarAccount.timezone;
    ...
    if (object._dashboardRequest.status == 1) {
      object.viewUserList(
        object._dashboardRequest.month,
        object._dashboardRequest.day,
        object._dashboardRequest.year,
        calendarData,
        accountKey,
        true,
        function (callback) {
  ...
  this.viewUserList = function (
    ...
    edit_title.textContent = object._calendar.formatBookingDate(
      month,
      day,
      year,
      null,
      null,
      null,
      calendarData.calendar[calendarKey].week,
      "text"
    );
    ...
    document
      .getElementById("menu_panel")
      .setAttribute("class", "media_frame_menu hidden_panel");
    document
      .getElementById("media_title")
      .setAttribute("class", "media_left_zero");
    document
      .getElementById("media_router")
      .setAttribute("class", "media_left_zero");
    document
      .getElementById("frame_toolbar")
      .setAttribute("class", "media_frame_toolbar media_left_zero");
      ...
      if (
        ...
        if (response.updateDate != null) {
          if (response.account.type == "day") {
            object.viewUserList(
              response.updateDate.month,
              response.updateDate.day,
              response.updateDate.year,
              response,
              accountKey,
              true,
              callback
    ...
    var changeButtonAction = function (
      buttonAction,
      mode,
      month,
      day,
      ...
      if (
        ...
        buttonAction == "add_reservation" ||
        buttonAction == "updateSchedule"
      ) {
        if (mode == 0) {
          day--;
          if (day == 0) {
        ...
        if (calendarData.account.type == "day") {
          ...
          object._console.log(
            "selectedKey = " +
              selectedKey +
              " count = " +
              calendarData.reservation[calendarKey].length
          );
        } else {
          ...
          reservationList =
            calendarData.reservationForHotel[visitorsList[selectedKey].checkIn][
              selectedKey
            ];
    ...
    if (
      document.getElementById("reservation_usersPanel") != null &&
      document.getElementById("add_reservationPanel") != null
    ) {
      reservation_usersPanel = document.getElementById(
        ...
        for (var key in menuTabList) {
          ...
          if (clickKey == "reservation_users") {
            ...
            if (document.getElementById("userInfoPanel") != null) {
              ...
              if (
                userPanel.getAttribute("class") == "show_panel_for_new_booking"
              ) {
            ...
            if (document.getElementById("changePanel") != null) {
              ...
              if (
                changePanel.getAttribute("class") == "show_change_panel" ||
                changePanel.getAttribute("class") == "return_change_panel"
              ) {
            ...
            var blockPanelCount =
              object._contentPanel.getElementsByClassName("blockPanel");
    ...
    if (object._buttonAction != "updateSchedule") {
      ...
      beforButton.setAttribute(
        "class",
        "material-icons button media-button button-primary button-large media-button-insert"
      );
      ...
      beforChangeButton.setAttribute(
        "class",
        "beforButton button media-button button-primary button-large media-button-insert hidden_panel"
      );
      ...
      nextChangeButton.setAttribute(
        "class",
        "nextButton button media-button button-primary button-large media-button-insert hidden_panel"
      );
      ...
      setTimeout(() => {
        const reserveButton = document.querySelector(
          "#rightButtonPanel .button.media-button.button-primary"
        );
        if (reserveButton) {
      ...
      function (response) {
        ...
        button.setAttribute(
          ...
          "changeStatusForDashboard(this, " +
            ...
            visitor.accountKey +
            ", '" +
            response.bookingStatus.toLowerCase() +
            "', " +
            visitor.date.month +
            ", " +
            visitor.date.day +
  ...
  this.changeStatus = function (
    ...
    confirm.selectPanelShow(
      object._i18n.get("Update status"),
      selectButtonList,
      status,
      enable,
      function (newStatus) {
        ...
        if (newStatus != false) {
          ...
          if (
            ...
            parseInt(object._emailEnableList.mail_approved.enable) == 1
          ) {
            object._console.log(
              "mail_pending = " +
                Boolean(parseInt(object._emailEnableList.mail_approved.enable))
            );
            enable = false;
          } else if (
            newStatus.toUpperCase() == "PENDING" &&
            parseInt(object._emailEnableList.mail_pending.enable) == 1
          ) {
            object._console.log(
              "mail_approved = " +
                Boolean(parseInt(object._emailEnableList.mail_pending.enable))
            );
            enable = false;
          } else if (
            newStatus.toUpperCase() == "CANCELED" &&
            parseInt(
              object._emailEnableList.mail_canceled_by_visitor_user.enable
            ) == 1
          ) {
            object._console.log(
              "mail_approved = " +
          ...
          confirm.dialogPanelShow(
            object._i18n.get("Attention"),
            object._i18n.get(
              "Will emails be sent to both customers and administrators?"
            ),
            enable,
            0,
            function (sendEmail) {
              ...
              object._loadingPanel.setAttribute(
                "class",
  ...
  this.changeDisplayFormatTab = function (
    reservation_usersPanel,
    month,
    day,
    year,
  ...
  this.reservation_users = function (
    reservation_usersPanel,
    month,
    day,
    year,
    ...
    var createRow = function (
      ...
      if (calendarData.account.type === "day") {
        ...
        th.textContent = object._calendar.getPrintTime(
          customer.date.hour,
          customer.date.min
        );
        tr.appendChild(th);
      ...
      if (customer.status != "canceled") {
        status.onclick = function () {
          ...
          object._console.log(
            "enable = " +
              Boolean(parseInt(object._emailEnableList.mail_new_admin.enable))
          );
    ...
    if (calendarData.account.type == "day") {
      if (reservationList != null) {
        if (object._changeDisplayFormatBookedCustomersForDay === "list") {
          ...
          // 30分刻みのループ（9:30〜17:00）
          for (let t = 9.5; t <= 17.0; t += 0.5) {
              const hour = Math.floor(t);
              const minute = t % 1 === 0 ? "00" : "30";

              const timeStr = String(hour).padStart(2, "0") + ":" + minute;
              const gridRowStart = Math.floor(t * 60);
          ...
          // ★10:45枠だけ追加
          let timeSlot = document.createElement("div");
          timeSlot.setAttribute("style", "grid-row-start: 645;");
          timeSlot.classList.add("timeSlotForDay");
          timeSlot.textContent = "10:45";

          let customerBox = document.createElement("div");
          customerBox.setAttribute("style", "grid-row-start: 645;");
          customerBox.classList.add("customerBoxForDay");
          ...
          const addCustomerInTimeSlots = function (
            start,
            end,
            customer,
            gridCustomers
          ...
          function createCustomerBoxForDayGrid() {
              ...
              for (let row = start; row <= end; row += 30) {
                  for (let col = 2; col <= 4; col++) {
                      ...
                      box.setAttribute(
                          "style",
                          `grid-row-start: ${row}; grid-column-start: ${col}; grid-column-end: ${col + 1};`
                      );
              ...
              // ★ 追加：10:45枠（645分）
              const specialRow = 645;
              for (let col = 2; col <= 4; col++) {
                  const box = document.createElement("div");
                  box.classList.add("customerBoxForDay");
                  box.setAttribute(
                      "style",
                      `grid-row-start: ${specialRow}; grid-column-start: ${col}; grid-column-end: ${col + 1};`
                  );
                  timeTablePanel.appendChild(box);
              }
          }
          ...
          for (var i = 0; i < reservationList.length; i++) {
            ...
            object._console.log(customer);
            let startMinutes =
              parseInt(customer.date.hour) * 60 + parseInt(customer.date.min);
            ...
            let result = addCustomerInTimeSlots(
              startMinutes,
              endMinutes,
              customer,
              gridCustomers
            ...
            const salesValue = customer.praivateData?.[salesIndex]?.value;
            if (salesValue) {
              extraInfo.push("売上: " + salesValue);
            }

            const emailValue = customer.praivateData?.[emailIndex]?.value;
            if (!emailValue || emailValue.trim() === "") {
              extraInfo.push("メアド未記入");
            }

            const memoValue = customer.praivateData?.[memoIndex]?.value;
            if (memoValue) {
              extraInfo.push(typeof memoValue === "string" ? memoValue.replace(/\\/g, "") : memoValue);
            }

            const extraInfoText = extraInfo.filter(Boolean).join("\n");
              ...
              if (customer.options.some((opt) => opt.name.includes("初診"))) {
                ...
                category = "other"; // 4列目 → スターラックス含む
              }             

              // ▼ 診察券番号の抽出（再診・スターラックス）
              let patientId = "";
              if (category === "first") {
                patientId = "ID: " + customer.key;  // 初診は予約ID
              } else {
                const patientIdItem = (customer.praivateData || []).find(item =>
                  item.name === "診察券番号" || item.id === "name"
                );
                if (patientIdItem && patientIdItem.value) {
                  patientId = "ID: " + patientIdItem.value;
                }
              }

              // ▼ サービス名を短縮表示（最大7文字）
              let servicesSummary = serviceDetails.services.join(", ");
              if (servicesSummary.length > 7) {
                servicesSummary = servicesSummary.substring(0, 7) + "...";
              }
            ...
            //   category = "first";
            // } else if (
            //   customer.options.some((opt) => opt.name.includes("再診"))
            // ) {
            //   category = "repeat";
            // } else {
            //   category = "other";
            ...
            if (!window.bookingGroupWrappers[groupKey]) {
              ...
              // 対応するboxに追加
              let matchedBox = Array.from(
                document.querySelectorAll(".customerBoxForDay")
              ).find((cell) => {
            ...
            customerPanel.setAttribute(
              "class",
              statusClassName + " bookedCustomerPanel"
            );
      ...
      toDay = new Date(
        calendarArray[startKey]
          .toString()
          .replace(/^(\d{4})(\d{2})(\d{2})$/, "$1-$2-$3") + "T00:00:00+00:00"
      );
      ...
      if (object._changeDisplayFormatBookedCustomersForHotel === "table") {
        ...
        for (var i = startKey; i < endKey; i++) {
          ...
          (function (reservationList, date, start, nights, visitors) {
            ...
            for (var key in reservationList) {
              ...
              if (visitors[id] != null) {
                ...
                visitors[id].accommodationDetails =
                ...
                visitors[id] = {
                  ...
                  displayNights:
                    reservation.accommodationDetails.nights - start,
                  name: name,
                  startDay: startDay,
                  endDay: endDay,
                  ...
                  checkOutBool: 0,
                  accommodationDetails: reservation.accommodationDetails,
                };
              }
            }
          ...
          th.setAttribute("scope", "row");
          th.textContent = object._calendar.formatBookingDate(
            null,
            calendarInfo.day,
            null,
            null,
        ...
        for (var key in visitors) {
          ...
          visitorPanel.textContent =
            "ID: " + visitors[key].key + " " + visitors[key].name;
        ...
        for (var key in reservationList[calendarKey]) {
          ...
          if (
            parseInt(checkIn.month) === month &&
            parseInt(checkIn.day) === day &&
            parseInt(checkIn.year) === year
          ) {
            let tr = createRow(
              key,
              bookedCustomer,
    ...
    if (object._dashboardRequest.status == 1) {
      ...
      if (calendarData.account.type == "day") {
        for (var i = 0; i < reservationList.length; i++) {
          ...
          if (
            parseInt(userInfo.key) == parseInt(object._dashboardRequest.key)
          ) {
            key = i;
        ...
        var checkIn = object._calendar.getDateKey(
          object._dashboardRequest.month,
          object._dashboardRequest.day,
          object._dashboardRequest.year
        );
  ...
  this.showUserInfo = function (
    ...
    var date = object._calendar.formatBookingDate(
      reservationDate.month,
      reservationDate.day,
      reservationDate.year,
      reservationDate.hour,
      reservationDate.min,
      reservationData.scheduleTitle,
      reservationData.date.week,
      "text"
    );
    ...
    var response = object.showUserDetails(
      key,
      calendarData,
      reservationData,
      accountKey,
    ...
    if (
      typeof userInfoPanel == "object" &&
      typeof userInfoPanel.scrollTo == "function"
    ) {
    ...
    var cancelToEdit = function (closePanel) {
      ...
      document
        .getElementById("downloadCsvForDay")
        .classList.remove("hidden_panel");
      document
        .getElementById("beforChangeButton")
      ...
      var date = object._calendar.formatBookingDate(
        ...
        reservationData.date.hour,
        reservationData.date.min,
        reservationData.scheduleTitle,
        reservationData.date.week,
        "text"
      );
      if (document.getElementById("booking_date") != null) {
        ...
        dateLabel.setAttribute(
          "data-unixTime",
          reservationData.scheduleUnixTime
        );
        dateLabel.textContent = date;
      }

      if (
        reservationData.courseKey != null &&
        reservationData.courseTime != null &&
        document.getElementById("booking_course") != null
      ) {
        object._console.log(reservationData);
      ...
      if (document.getElementById("booking_cost") != null) {
        var cost = object._format.formatCost(
          reservationData.courseCost,
      ...
      if (calendarData.account.type == "hotel") {
        ...
        for (var roomKey in visitorsDetails.rooms) {
          for (var key in guestsList) {
            ...
            var guestsParentPanel = document.getElementById(
              "guests_" + roomKey + "_" + key
            ).parentElement;
            var guestsPanel = document.getElementById(
              "guests_" + roomKey + "_" + key
            );
            var guestsSelectPanel = document.getElementById(
              "select_guests_" + roomKey + "_" + key
            );
        ...
        var totalNumberOfGuests =
          parseInt(reservationData.accommodationDetails.adult) +
          parseInt(reservationData.accommodationDetails.children);
    ...
    if (blockPanel != null) {
      blockPanel.onclick = function (event) {
        ...
        if (document.getElementById("changePanel") != null) {
          document
            .getElementById("changePanel")
            .setAttribute("class", "return_panel");
        }
    ...
    editButton.onclick = function () {
              ...
              for (var key in hotelOptions) {
                ...
                hotelOptions[key].json = (function (options, selectedOption) {
                  ...
                  for (var key in options) {
                    ...
                    if (
                      parseInt(options[key].index) ==
                      parseInt(selectedOption.index)
                    ) {
                ...
                var value = input.createInput(
                  ...
                  function (event) {
                    ...
                    object._console.log(hotelOptions[key]);
                    var response = object._hotel.updateSelectedOptions(
                      key,
                      index,
                      roomKey,
                      true
                    ...
                    if (
                      verifyGuests.booking == false ||
                      response.nights == 0 ||
                      verifyGuests.requiredGuests == false ||
                      verifyGuests.requiredOptions == false
                    ) {
                ...
                var optionsParentPanel = document.getElementById(
                  "options_" + roomKey + "_" + key
                ).parentElement;
                var optionsPanel = document.getElementById(
                  "options_" + roomKey + "_" + key
                );
            ...
            for (var key in guestsList) {
              ...
              guestsList[key].json = (function (guests, valueGuest) {
                ...
                for (var key in guests) {
                  ...
                  if (
                    parseInt(guests[key].number) == parseInt(valueGuest.number)
                  ) {
              ...
              var value = input.createInput(
                ...
                function (event) {
                  ...
                  var roomKey = this.parentElement.getAttribute("data-room");
                  var index = parseInt(this.selectedIndex);
                  var response = object._hotel.updateSelectedGuests(
                    key,
                    index,
                    roomKey,
                    true
      ...
      for (var i = 0; i < formPanelList.length; i++) {
        ...
        var value = input.createInput(i, form, inputData, null);
        var rowPanel = object.createRowPanel(
          form.name,
          value,
          null,
          form.required,
      ...
      saveButton.onclick = function () {
        ...
        var schedule = {
          key: reservationData.scheduleKey,
          unixTime: reservationData.scheduleUnixTime,
        };
        ...
        if (document.getElementById("booking_course") != null) {
          var booking_course = document
            .getElementById("booking_course")
            .getAttribute("data-key");
        ...
        if (typeof visitorServices == "object") {
          ...
          for (var key in object._courseList) {
            ...
            for (var visitorKey in visitorServices) {
              ...
              if (
                parseInt(service.key) == parseInt(visitorService.key) &&
                parseInt(visitorService.selected) == 1
              ) {
        ...
        if (post !== false) {
          ...
          if (parseInt(object._emailEnableList.mail_updated.enable) == 1) {
            object._console.log(
              "mail_pending = " +
                Boolean(parseInt(object._emailEnableList.mail_updated.enable))
            );
            enable = false;
          }
      ...
      returnButton.onclick = function () {
        ...
        if (
          document.getElementById("changePanel") != null &&
          document.getElementById("changePanel").getAttribute("class") !=
            "return_panel"
        ) {
          document
            .getElementById("changePanel")
            .setAttribute("class", "return_change_panel");
        }
        object._contentPanel.removeChild(blockPanel);
    ...
    deleteButton.onclick = function () {
      ...
      object._console.log(
        "enable = " +
          Boolean(parseInt(object._emailEnableList.mail_deleted.enable))
      );

      var enable = false;
      if (
        Boolean(parseInt(object._emailEnableList.mail_deleted.enable)) === false
      ) {
        enable = true;
      }
      object._console.log("enable = " + enable);
      ...
      if (
        reservationData.payToken != null &&
        reservationData.payToken.length != 0 &&
        reservationData.payMode == "CreditCard"
      ) {
        refoundBool = false;
      }
      ...
      confirm.dialogPanelShow(
        object._i18n.get("Attention"),
        object._i18n.get(
          "This booking was paid through %s. Will the payment be refunded to the customer?",
          [reservationData.payName]
        ),
        refoundBool,
        0,
        function (refoundValue) {
    ...
    bookingTimeChange.onclick = function () {
      ...
      if (object._isExtensionsValid == 0) {
        confirm.alertPanelShow(
          object._i18n.get("Warning"),
          object._i18n.get("Paid plan subscription required."),
          false,
          null
        );
        return null;
          ...
          function (response) {
            ...
            if (response.status == "statusButton") {
              ...
              document
                ...
                .classList.add("hidden_panel");
              document
                .getElementById("nextChangeButton")
                .classList.add("hidden_panel");
    ...
    function hideSpecialRows(retry = 0) {
      const planNames = [
        ...document.querySelectorAll(".courseLinePanel .planName"),
      ].map((el) => el.textContent);

      const hasReshin = planNames.some(
        (text) => text.includes("再診") || text.includes("スターラックス")
      );
      const hasShoshinOrStarLux = planNames.some(
        (text) => text.includes("初診")
      );

      // ▼ ID行（元々IDというラベルの行）を特定
      const idRow = [...document.querySelectorAll("#inputFormPanel .row")].find(
        (row) => {
      ...
      if (idRow) {
        ...
        if (hasReshin) {
          // #inputFormPanel が #userInfoPanel 内に存在するか確認
          const userInfoPanel = document.getElementById("userInfoPanel");
          const isInsideUserInfo = userInfoPanel && userInfoPanel.contains(idRow);
        
          if (isInsideUserInfo) {
            idRow.classList.add("hidden_panel");
            // console.log('✅ 再診・スターラックスあり → userInfoPanel内のみ非表示');
          }
        }
      }
      ...
      if (hasShoshinOrStarLux) {
        const rows = [
          ...document.querySelectorAll("#inputFormPanel .row"),
        ].filter((row) => {
          const nameEl = row.querySelector(".name");
  ...
  this.showUserDetails = function (
    ...
    // ▼ 再診 or スターラックスのときに診察券番号入力行を強制追加
    (function addTicketNumberRowIfNeeded() {
      const planNames = [
        ...document.querySelectorAll(".courseLinePanel .planName"),
      ].map((el) => el.textContent);

      const hasReshinOrStarLux = planNames.some(
        (text) => text.includes("再診") || text.includes("スターラックス")
      );
      if (!hasReshinOrStarLux) return;

      const target = document.querySelector("#inputFormPanel");
      if (!target || target.querySelector("#input_number")) return; // 重複回避

      const html = `
        <div class="row">
          <div class="name">診察券番号</div>
          <div id="number" class="value">
            <div id="value_number" class="hidden_panel"></div>
            <input type="text" id="input_number" class="regular-text form_text" placeholder="番号がわからない場合は「不明」とご記入ください">
    ...
    var date = object._calendar.formatBookingDate(
      ...
      reservationDate.min,
      reservationData.scheduleTitle,
      reservationDate.week,
      "text"
    );

    var timestampDate = reservationData.timestamp;
    var timestamp = object._calendar.formatBookingDate(
      timestampDate.month,
      timestampDate.day,
      timestampDate.year,
      timestampDate.hour,
    ...
    if (showStatusForHotel === true) {
      var statusPanel = object.createRowPanel(
        object._i18n.get("Status"),
        "",
        "status",
        "false",
        null
    ...
    if (calendarData.account.type == "hotel") {
      ...
      object._hotel.setCallback(null);
      object._hotel.setCalendarAccount(calendarData.account);
      object._hotel.setNights(
        parseInt(reservationData.accommodationDetails.nights)
      );
      ...
      var visitorsScheduleList =
        reservationData.accommodationDetails.scheduleDetails;
      ...
      object._hotel.setCheckIn(
        reservationData.accommodationDetails.checkInSchedule
      );
      object._hotel.setCheckInKey(
        reservationData.accommodationDetails.checkInSchedule.key
      );
      object._hotel.setCheckOut(
        reservationData.accommodationDetails.checkOutSchedule
      );
      object._hotel.setCheckOutKey(
        reservationData.accommodationDetails.checkOutSchedule.key
      );
      ...
      var expressionsCheck = object._calendar.getExpressionsCheck(
        calendarData.account,
        false
      );

      var date = object._calendar.formatBookingDate(
        reservationData.date.checkIn_month,
        reservationData.date.checkIn_day,
        reservationData.date.checkIn_year,
        null,
        null,
        null,
        reservationData.date.checkIn_week,
        "text"
      );
      var checkIn = object.createRowPanel(
        object._i18n.get("Check-in"),
        date,
        "checkIn",
        "false",
        null
      ...
      document
        .getElementById("checkIn")
        .setAttribute("data-id", reservationData.scheduleKey);

      date = object._calendar.formatBookingDate(
        reservationData.date.checkOut_month,
        reservationData.date.checkOut_day,
        reservationData.date.checkOut_year,
        null,
        null,
        null,
        reservationData.date.checkOut_week,
        "text"
      );
      var checkOut = object.createRowPanel(
        object._i18n.get("Check-out"),
        date,
        "checkOut",
        "false",
        null
      ...
      var nightsValue =
        reservationData.accommodationDetails.nights +
        " " +
        object._i18n.get("nights");
      if (parseInt(object._calendarAccount.formatNightDay) == 1) {
        nightsValue = object._i18n.get("%s nights %s days", [
          reservationData.accommodationDetails.nights,
          reservationData.accommodationDetails.nights + 1,
        ]);
      }
      ...
      if (reservationData.accommodationDetails.nights == 1) {
        nightsValue =
          reservationData.accommodationDetails.nights +
          " " +
          object._i18n.get("night");
        if (parseInt(object._calendarAccount.formatNightDay) == 1) {
          nightsValue = object._i18n.get("%s night %s days", [
            reservationData.accommodationDetails.nights,
            reservationData.accommodationDetails.nights + 1,
          ]);
        }
      }
      var totalLengthOfStay = object.createRowPanel(
        object._i18n.get("Total length of stay"),
        nightsValue,
        null,
        null,
        null
      ...
      for (var roomKey in reservationData.accommodationDetails.rooms) {
        ...
        roomNoLabel.textContent =
          object._i18n.get("Room") + ": " + (roomNumber + 1);
          ...
          for (var key in hotelOptions) {
            ...
            for (var i = 0; i < list.length; i++) {
              ...
              if (parseInt(list[i].selected) == 1) {
                ...
                var response = object._hotel.addSelectedOptions(
                  key,
                  list[i].index,
                  parseInt(roomKey)
                );
            ...
            var optionsPanel = object.createRowPanel(
              hotelOptions[key].name,
              value,
              "options_" + roomKey + "_" + hotelOptions[key].key,
              "false",
              null
            );
          ...
          for (var key in guestsList) {
            ...
            for (var i = 0; i < list.length; i++) {
              if (parseInt(list[i].selected) == 1) {
                ...
                var response = object._hotel.setGuests(
                  key,
                  i,
                  list[i].number,
                  parseInt(roomKey)
            ...
            var guestsPanel = object.createRowPanel(
              guestsList[key].name,
              value,
              "guests_" + roomKey + "_" + guestsList[key].key,
              "false",
              null
            );
      ...
      if (
        hasOptionsAndGuestsInRoom.options === false &&
        hasOptionsAndGuestsInRoom.guests === false
      ) {
      ...
      if (
        reservationData.accommodationDetails.adult != null &&
        reservationData.accommodationDetails.children != null
      ) {
        var totalNumberOfGuests =
          parseInt(reservationData.accommodationDetails.adult) +
          parseInt(reservationData.accommodationDetails.children);
        ...
        if (totalNumberOfGuests > 1) {
          ...
          totalNumberOfGuestsValue = object._i18n.get("%s guests", [
            totalNumberOfGuests,
          ]);
        } else if (totalNumberOfGuests == 1) {
          //totalNumberOfGuestsValue = totalNumberOfGuests + " " + object._i18n.get("person");
          totalNumberOfGuestsValue = object._i18n.get("%s guest", [
            totalNumberOfGuests,
          ]);
        }

        totalNumberOfGuestsPanel = object.createRowPanel(
          object._i18n.get("Total number of guests"),
          String(totalNumberOfGuestsValue),
          "totalGuests",
          null,
          null
        );
      ...
      var summaryPanel = object.createRowPanel(
        object._i18n.get("Summary"),
        summaryListPanel,
        "summaryPanel",
        null,
        null
      ...
      for (var i = 0; i < reservationData.taxes.length; i++) {
        ...
        if (
          (tax.type == "tax" && tax.tax == "tax_exclusive") ||
          tax.type == "surcharge"
        ) {
      ...
      var totalPrice = object._format.formatCost(
        reservationData.accommodationDetails.totalCost,
        object._currency
      );
      object._console.log("totalPrice = " + totalPrice);

      var rowPanel = object.createRowPanel(
        object._i18n.get("Total amount"),
        totalPrice,
        "totalPrice",
        null,
        null
      ...
      document
        .getElementById("booking_date")
        .setAttribute("data-key", reservationData.scheduleKey);
      document
        .getElementById("booking_date")
        .setAttribute("data-unixTime", reservationData.scheduleUnixTime);
      var guests = [];
      ...
      //var responseGuests = [];
      var responseGuests =
        object._servicesControl.getValueReflectGuests(guests);
      ...
      if (reservationData.options.length > 0) {
        ...
        if (parseInt(reservationData.courseCost) > 0) {
          ...
          courseCostPanel.textContent = object._format.formatCost(
            reservationData.courseCost,
            reservationData.currency
          );
        }
        ...
        if (reservationData.options != "") {
          if (hasMultipleServices == true) {
            ...
            var response = object.createServicesPanel(
              coursePanel,
              services,
              "options",
              guests,
              ...
              for (var i = 0; i < options.length; i++) {
                ...
                if (parseInt(option.selected) == 1) {
                  ...
                  if (parseInt(option.cost) > 0) {
                    optionPricePanel.textContent = object._format.formatCost(
                      option.cost,
                      reservationData.currency
                    );
                  }
        ...
        document
          .getElementById("booking_course")
          .setAttribute("data-key", reservationData.courseKey);
        document
          .getElementById("booking_course")
          .setAttribute("data-time", reservationData.courseTime);
        if (object.getServiceOrOption(reservationData.options) == "service") {
          document
            .getElementById("booking_course")
            .setAttribute("data-key", "multipleServices");
        }
      }
      ...
      object._console.log("cost = " + cost);
      if (coupon.key != null) {
        var couponPanel = object.createRowPanel(
          object._i18n.get("Coupon"),
          "",
          null,
          null,
          null
        ...
        if (coupon.method == "subtraction") {
          valuePanel.textContent = object._format.formatCost(
            coupon.value,
            object._currency
          );
      ...
      var taxes = new TAXES(
        ...
        reservationData.currency,
        object._debug,
        object._numberFormatter,
        object._currency_info
      );
      var surchargePanel =
        taxes.createExtraChargesAndTaxesElement("surchargeTaxTitle");
      ...
      var isTaxes = taxes.taxesDetails(
        cost,
        formPanel,
        surchargePanel,
        taxePanel,
      ...
      if (isTaxes.isTaxes === true) {
        ...
        cost += taxes.reflectTaxesInTotalCost(
          responseTaxes,
          goodsList,
          reflectAdditional
        );
    ...
    if (reservationData.payId == "stripe") {
      ...
      paymentMethodValue = object._i18n.get(
        "Pay at a convenience store with Stripe"
      );
    } else if (reservationData.payId == "paypal") {
      paymentMethodValue = object._i18n.get("Pay with PayPal");
    }

    var paymentMethodPanel = object.createRowPanel(
      object._i18n.get("Payment method"),
      paymentMethodValue,
      "payment_method",
      null,
      null
    ...
    if (
      reservationData.payId == "stripe" ||
      reservationData.payId == "stripe_konbini" ||
      reservationData.payId == "paypal"
    ) {
      //var rowPanel = object.createRowPanel("Charge ID of " + reservationData.payName, reservationData.payToken, "booking_cost", null, null);
      var paymentIdPanel = object.createRowPanel(
        object._i18n.get("Payment ID"),
        reservationData.payToken,
        "booking_cost",
        null,
        null
      );
    ...
    var timestampPanel = object.createRowPanel(
      object._i18n.get("Submission date"),
      timestamp,
      "timestamp",
      null,
      null
  ...
  this.getDiscountCostByCoupon = function (coupon, totalCost) {
    ...
    if (coupon.method == "subtraction") {
      ...
      totalCost -=
        totalCost - (totalCost * (100 - parseInt(coupon.value))) / 100;
    }
  ...
  this.showGuestsPanelForDays = function (formPanel, guests) {
    ...
    if (responseGuests.totalNumberOfGuests > 0) {
      var totalNumberOfGuestsPanel = object.createRowPanel(
        object._i18n.get("Total number of guests"),
        responseGuests.totalNumberOfGuestsTitle,
        object._prefix + "totalNumberOfGuests",
        null,
        null
      );
      formPanel.appendChild(totalNumberOfGuestsPanel);
  ...
  this.createToChangePanelForTimeOrCourse = function (
    month,
    day,
    year,
    reservationData,
    ...
    var response = object.preparToUpdateSchedule(
      reservationData,
      calendarData,
      parseInt(courseTime),
      changeAction,
    ...
    if (changeAction == "date") {
      if (options != null) {
        updateSchedule[calendarKey] = object._servicesControl.invalidService(
          updateSchedule[calendarKey],
          calendarData["bookedServices"],
          selectedService,
          courseTime
        );
      }
      ...
      if (
        object._nationalHoliday[calendarKey] != null &&
        parseInt(object._nationalHoliday[calendarKey].status) == 1
      ) {
      ...
      for (var i = 0; i < updateSchedule[calendarKey].length; i++) {
        ...
        if (
          timeToProvide[week] != null &&
          parseInt(timeToProvide[week][minutes]) == 0
        ) {
          schedule["select"] = false;
        }
        ...
        schedulePanel.textContent =
          object._calendar.getPrintTime(
            ("0" + schedule["hour"]).slice(-2),
            ("0" + schedule["min"]).slice(-2)
          ) + schedule["title"];
        ...
        if (schedule["select"] === true) {
          ...
          schedulePanel.onclick = function () {
            this.setAttribute(
              "class",
              "courseAndScheduleRow courseAndScheduleRowActive"
            );
            var key = this.getAttribute("data-key");
            var schedule = calendarData["schedule"][calendarKey][key];
            object.unselectPanel(
              key,
              scheduleListPanel,
              "courseAndScheduleRow"
            );
            object._console.log(schedule);
      ...
      for (var i = 0; i < object._courseList.length; i++) {
        ...
        var responseCosts = object._servicesControl.getCostsInService(
          courseData,
          guests,
          isGuests,
          object._isExtensionsValid
        );
        ...
        if (
          timeToProvide[weekKey] != null &&
          parseInt(timeToProvide[weekKey][minutes]) == 0
        ) {
          courseData["select"] = false;
        }
            ...
            if (options != null && options.length > 0) {
              if (checkBox.checked == true) {
                var selectOptionsPanel =
                  document.getElementById("selectOptionsPanel");
                object.getOptionsPanel(
                  courseData,
                  options,
                  guests,
                  false,
                  function (response) {
                    ...
                    object._courseList[key]["selectedOptionsList"] =
                      selectedOptions;
                    var selectedServices = object.addVisitorServices(
                      object._courseList[parseInt(key)]
                    );
                    object._console.log(selectedServices);
                    object.createServicesPanel(
                      coursePanel,
                      selectedServices,
                      "selectedOptionsList",
                      guests,
                      parseInt(object._calendarAccount.guestsBool),
                      [],
                      reservationData.currency,
                      true
                    );
  ...
  this.preparToUpdateSchedule = function (
    reservationData,
    calendarData,
    courseTime,
    changeAction,
    ...
    if (
      ...
      if (
        preparation.position == "before_after" ||
        preparation.position == "before"
      ) {
  ...
  this.add_reservation = function (
    ...
    const observer = new MutationObserver(() => {
      setTimeout(() => {
        ...
        planNameEls.forEach((el) => {
          ...
          if (el.textContent.includes("再診") || el.textContent.includes("スターラックス")
          ) {
            hasKeyword = false;
          }
        });
    
        // 初診や選択なしの場合に診察券番号を非表示にする
        if (hasKeyword) {
          const idRow = document.querySelector(".row[data-key='1']");
          if (idRow && !idRow.classList.contains("hidden_panel")) {
            idRow.classList.add("hidden_panel");
            console.log("🟡 診察券番号の行を非表示にしました");
          }
        } else {
          // 再診などのときは表示（消されっぱなし防止）
          const idRow = document.querySelector(".row[data-key='1']");
          if (idRow && idRow.classList.contains("hidden_panel")) {
            idRow.classList.remove("hidden_panel");
            console.log("🟢 診察券番号の行を再表示しました");
          }
        }
      }, 100); // DOM構築後を確実に狙う
    });
    ...
    scheduleMainPanel.setAttribute(
      "class",
      "scheduleListPanel courseListPanel box_shadow"
    );
    ...
    if (calendarData.account.type == "hotel") {
      ...
      var calendar = new Booking_App_Calendar(
        weekName,
        object._dateFormat,
        object._positionOfWeek,
        object._positionTimeDate,
      ...
      if (
        checkDate.checkIn == null &&
        calendarData.schedule[parseInt(calendarKey)] != null &&
        calendarData.schedule[parseInt(calendarKey)][0] != null
      ) {
      ...
      var bookingCalendar = function (calendarData) {
        ...
        object._calendar.create(
          ...
          function (calendarCallback) {
            ...
            if (
              calendarData.schedule[key] != null &&
              calendarData.schedule[key][0] != null
            ) {
              dayPanel.setAttribute(
                "data-unixTime",
                calendarData.schedule[key][0].unixTime
              );
              ...
              if (calendarData.schedule[key].length != 0) {
                ...
                if (
                  checkDate.checkIn != null &&
                  checkDate.checkIn.unixTime ==
                    calendarData.schedule[key][0].unixTime &&
                  calendarData.schedule[key][0].remainder > 0
                ) {
                ...
                if (
                  checkDate.checkIn != null &&
                  checkDate.checkOut != null &&
                  checkDate.checkIn.unixTime <=
                    calendarData.schedule[key][0].unixTime &&
                  checkDate.checkOut.unixTime >=
                    calendarData.schedule[key][0].unixTime
                ) {
                  object._console.log(
                    "checkDate.checkIn.unixTime = " + checkDate.checkIn.unixTime
                  );
                  dayPanel.classList.add("selected_day_slot");
                }

                if (
                  parseInt(calendarData.schedule[key][0].remainder) <= 0 ||
                  calendarData.schedule[key][0].stop == "true"
                ) {
                ...
                dayPanel.onclick = function () {
                  ...
                  var checkInUnixTime = parseInt(
                    document
                      .getElementById("checkIn")
                      .getAttribute("data-unixTime")
                  );
                  object._console.log(
                    "checkIn = " +
                      checkIn +
                  ...
                  if (checkDate.checkIn == null) {
                    ...
                    var checkInClearLabel =
                      document.getElementById("checkInClear");
                    ...
                    if (checkInUnixTime < checkOutUnixTime) {
                      ...
                      for (var key in dayPanelList) {
                        var checkOut = parseInt(
                          dayPanelList[key].getAttribute("data-unixTime")
                        );
                        if (
                          verifySchedule.status == true &&
                          checkInUnixTime <= checkOut &&
                          checkOutUnixTime >= checkOut
                        ) {
                          dayPanelList[key].classList.add("selected_day_slot");
                        } else if (checkInUnixTime != checkOut) {
                          dayPanelList[key].classList.remove(
                            "selected_day_slot"
                          );
                        }
                      }
                    ...
                    if (verifySchedule.status == true) {
                      ...
                      checkOut.textContent = object._calendar.formatBookingDate(
                        schedule.month,
                        schedule.day,
                        schedule.year,
                        null,
      ...
      if (object._courseBool == true) {
        ...
        var isBooking = object._servicesControl.validateServices(
          month,
          day,
          year,
          week,
        ...
        for (var i = 0; i < object._courseList.length; i++) {
            ...
            if (options.length > 0) {
              ...
              object.createOptionsPanel(
                ...
                function (response) {
                  ...
                  object._console.log(course);
                  object._console.log(response);
                  courseList[parseInt(response.key)].selectedOptionsList =
          ...
          var responseCosts = object._servicesControl.getCostsInService(
            course,
            calendarData.guestsList,
            parseInt(calendarData.account.guestsBool),
            object._isExtensionsValid
          );
          object._console.log(responseCosts);
          if (
            responseCosts.max != null &&
            isNaN(parseInt(responseCosts.max)) === false &&
            parseInt(responseCosts.max) != 0
          ) {
            var cost = object._format.formatCost(
              responseCosts.max,
              object._currency
            ...
            if (options.length <= 0) {
              ...
              if (checkBox.checked == true) {
                object.createOptionsPanel(
                  ...
                  accountKey,
                  function (response) {
                    courseList[parseInt(key)].selectedOptionsList =
                      response.selectedOptions;
                    object.createSchedule(
                      mainPanel,
                      courseMainPanel,
        ...
        if (coursePanelList[0] != null) {
          coursePanelList[0].setAttribute(
            "class",
            "courseAndScheduleRow courseAndScheduleRowActive"
          );
        }
  ...
  this.getOptionsPanel = function (
    ...
    var nextButton =
      selectOptionsPanel.getElementsByClassName("decisionButton")[0];
    ...
    if (closeWithClick === true) {
      selectOptionsPanel.getElementsByClassName("blockPanel")[0].onclick =
        function () {
  ...
  this.createOptionsPanel = function (
    mainPanel,
    courseMainPanel,
    scheduleMainPanel,
    formMainPanel,
  ...
  this.createBookingCalendar = function (
    add_reservationPanel,
    day,
    calendarData,
    accountKey,
          ...
          if (calendarData.schedule[key].length != 0) {
            ...
            dayPanel.onclick = function () {
              ...
              var checkIn = document
                ...
                .getAttribute("data-check");
              var checkInUnixTime = parseInt(
                document.getElementById("checkIn").getAttribute("data-unixTime")
              );
              object._console.log(
                "checkIn = " + checkIn + " checkInUnixTime = " + checkInUnixTime
              );
                ...
                for (var key in dayPanelList) {
                  if (day <= parseInt(key) && dateKey >= parseInt(key)) {
                    var remainder = parseInt(
                      calendarData.schedule[key][0].remainder
                    );
              ...
              if (checkOutBool == true) {
                ...
                checkOut.setAttribute("data-check", null);
                checkOut.textContent = object._i18n.get(
                  "Please choose a departure date from the calendar"
                );
              }
            };
          } else {
  ...
  this.createSchedule = function (
    ...
    for (var key in courseList) {
      if (
        courseList[key].selected == 1 &&
        courseList[key].timeToProvide != null
      ) {
        course = true;
      ...
      if (
        courseList[key].selected == 1 &&
        courseList[key].selectedOptionsList.length > 0
      ) {
        totalTimeInOptions += object.getTotalTimeInOptions(
          courseList[key].selectedOptionsList
        );
      }
    }
    object._console.log(timeToProvide);
    ...
    for (var i = 0; i < calendarData["schedule"][calendarKey].length; i++) {
      ...
      for (var key in timeToProvide) {
        if (
          timeToProvide[key][week] != null &&
          parseInt(timeToProvide[key][week][minutes]) == 0
        ) {
          schedule["select"] = false;
        }
      }
    ...
    if (course != null || object._preparationTime > 0) {
      var courseTime = 0;
      if (
        object._positionPreparationTime == "before_after" ||
        object._positionPreparationTime == "after"
      ) {
      ...
      if (course != null) {
        ...
        object._servicesControl.invalidService(
          calendarData["schedule"][calendarKey],
          calendarData["bookedServices"],
          selectedService,
          courseTime
        );
      ...
      if (
        object._positionPreparationTime == "before_after" ||
        object._positionPreparationTime == "before"
      ) {
      ...
      for (var i = 0; i < calendarData["schedule"][calendarKey].length; i++) {
        ...
        if (
          ...
          schedule["select"] = false;
          var rejectionTime =
            parseInt(schedule["hour"]) * 60 +
            parseInt(schedule["min"]) -
            courseTime;
          ...
          (function (
            ...
            for (var i = 0; i < schedule.length; i++) {
              var time =
                parseInt(schedule[i]["hour"]) * 60 +
                parseInt(schedule[i]["min"]);
              if (time > rejectionTime && i < key) {
                ...
                parseInt(schedule[i].unixTime) <= stopUnixTime &&
                i > key
              ) {
                object._console.log(
                  "i = " +
    ...
    for (var i = 0; i < calendarAccountList.length; i++) {
      if (
        parseInt(calendarAccountList[i].key) == parseInt(accountKey) &&
        parseInt(calendarAccountList[i].displayRemainingCapacity) == 1
      ) {
    ...
    object._console.log(
      "displayRemainingCapacity = " + displayRemainingCapacity
    );
    ...
    for (var i = 0; i < calendarData["schedule"][calendarKey].length; i++) {
      ...
      if (displayRemainingCapacity == 1) {
        ...
        displayRemainingCapacityLabel.classList.add(
          "displayRemainingCapacityLabel"
        );
        //displayRemainingCapacityLabel.textContent = object._i18n.get("%s remaining", [schedule.remainder]);
        displayRemainingCapacityLabel.textContent = object._i18n.get(
          "%s slots left",
          [schedule.remainder]
        );
        ...
        schedulePanel.onclick = function () {
          ...
          if (object.getGuestsList().length > 0) {
            ...
            for (var scheduleKey in calendarData["schedule"][calendarKey]) {
              var unixTime = parseInt(
                calendarData["schedule"][calendarKey][scheduleKey].unixTime
              );
              if (startUnixTime <= unixTime && endUnixTime > unixTime) {
                remainders.push(
                  parseInt(
                    calendarData["schedule"][calendarKey][scheduleKey].remainder
                  )
                );
                object._console.log(
                  calendarData["schedule"][calendarKey][scheduleKey]
                );
              }
            }
  ...
  this.createForm = function (
    ...
    if (object._courseBool == true) {
      ...
      if (calendarData.account.type == "hotel") {
        ...
        mainPanel.setAttribute(
          "class",
          "courseAndSchedulePanel_next_no_course"
        );
        //scheduleMainPanel.setAttribute("class", "schedulePanel_next");
      }
    }
    ...
    formMainPanel.setAttribute(
      "class",
      "createFormPanel_show box_shadow show_panel_for_new_booking test"
    );
    /**
        if(calendarData.account.type == "day"){
    ...
    if (parseInt(object._isExtensionsValid) == 1) {
      ...
      lookForUserButton.setAttribute(
        "class",
        "material-icons lookForUserButton"
      );
    ...
    if (user.meta != null) {
      ...
      var usernamePanel = object.createRowPanel(
        object._i18n.get("Username"),
        user.user_login,
        null,
        null,
        null
    ...
    if (calendarData.account.type == "hotel") {
      ...
      document
        .getElementById("checkIn")
        .setAttribute("data-check", schedule.key);
      document
        .getElementById("checkIn")
        .setAttribute("data-unixTime", schedule.unixTime);
      ...
      var checkOut = object.createRowPanel(
        expressionsCheck.departure,
        expressionsCheck.chooseDeparture,
        "checkOut",
        "true",
        checkOutClear
      );
      ...
      roomListPanel.classList.add("roomListPanel");
      formPanel.appendChild(roomListPanel);
      var totalNumberOfGuests = object.createRowPanel(
        object._i18n.get("Total number of guests"),
        "0",
        "totalGuests",
        null,
      ...
      var roomPanel = object.addGuestsForRoom(
        guestsList,
        hotelOptions,
        input,
        totalNumberOfGuests,
        0,
        object._calendarAccount.multipleRooms,
        false,
        bookingButton
      );
      roomListPanel.appendChild(roomPanel);
      ...
      if (parseInt(object._calendarAccount.multipleRooms) == 1) {
        ...
        object._console.log(
          "multipleRooms = " + object._calendarAccount.multipleRooms
        );
        ...
        addRoomButton.onclick = function (event) {
          ...
          if (visitorDetails.applicantCount < visitorDetails.vacancy) {
            ...
            var roomPanel = object.addGuestsForRoom(
              ...
              totalNumberOfGuests,
              roomNumber,
              parseInt(object._calendarAccount.multipleRooms),
              true,
              bookingButton
            );
      ...
      object._hotel.setCallback(function (response) {
        ...
        if (checkDate.checkOut == null) {
          ...
          if (checkDate.checkIn != null) {
            var dataKey = object._calendar.getDateKey(
              checkDate.checkIn.month,
              checkDate.checkIn.day,
              checkDate.checkIn.year
            );
        ...
        document.getElementById("totalLengthOfStayValue").textContent =
          nightsValue;
        ...
        var totalNumberOfGuestsValue = 0;
        if (totalNumberOfGuests > 1) {
          totalNumberOfGuestsValue =
            totalNumberOfGuests + " " + object._i18n.get("people");
        } else if (totalNumberOfGuests == 1) {
          totalNumberOfGuestsValue =
            totalNumberOfGuests + " " + object._i18n.get("person");
        }
        ...
        document.getElementById("totalPrice").textContent =
          object._format.formatCost(totalPrice, object._currency);
      ...
      if (parseInt(object._calendarAccount.guestsBool) == 1) {
        ...
        for (var key in guestsList) {
          ...
          for (var i = 0; i < list.length; i++) {
            if (
              maxApplicantCount >= parseInt(list[i].number) &&
              guests.guestsInCapacity == "included"
            ) {
              //values.push(list[i].name);
          ...
          var guestsSelectPanel = guestsInput.createInput(
            guestsList[key]["name"].replace(/\\/g, ""),
            guestsList[key],
            {},
            function (event) {
              var selectBox = this;
              object._console.log(selectBox);
              multipleApplicantCount = object._servicesControl.getSelectedGuest(
                guestsList,
                selectBox,
                multipleApplicantCountList
              );
              ...
              for (var guestsKey in guestsList) {
                if (guestsList[guestsKey].guestsInCapacity == "included") {
                  document
                    .getElementById(
                      object._prefix + "guests_" + guestsList[guestsKey].key
                    )
                    .classList.remove("error_empty_value");
                  var select = document.getElementById(
                    object._prefix + "input_guests_" + guestsList[guestsKey].key
                  );
              ...
              if (
                multipleApplicantCount > maxApplicantCount &&
                guests.guestsInCapacity == "included"
              ) {
              ...
              if (
                object._courseBool == true ||
                object._calendarAccount.flowOfBooking == "services"
              ) {
                var coursePanel = document.getElementById(
                  object._prefix + "selectedServicesPanel"
                );
              ...
              var responseGuests =
                object._servicesControl.getValueReflectGuests(guestsList);
              object._console.log(responseGuests);

              var totalNumberOfGuestsPanel = document.getElementById(
                object._prefix + "totalNumberOfGuests"
              );
              var totalNumberOfGuestsValuePanel =
                totalNumberOfGuestsPanel.getElementsByClassName("value")[0];
              var totalNumberOfGuestsErrorPanel =
                totalNumberOfGuestsPanel.getElementsByClassName(
                  "errorMessage"
                )[0];
              totalNumberOfGuestsValuePanel.textContent =
                responseGuests.totalNumberOfGuestsTitle;
              var responseLimitGuests =
                object._servicesControl.verifyToLimitGuests(
                  responseGuests,
                  object._calendarAccount.limitNumberOfGuests,
                  object._calendarAccount.type
                );
              ...
              if (responseLimitGuests.isGuests === true) {
                ...
                totalNumberOfGuestsErrorPanel.textContent =
                  responseLimitGuests.errorMessage;
              }

              var surchargePanel = document.getElementById(
                object._prefix + "surchargeTaxPanel"
              );
              var taxes = new TAXES(
                object._i18n,
                object._currency,
              ...
              if (isTaxes.isTaxes === true) {
                ...
                object._console.log("reflectAdditional = " + reflectAdditional);
                totalCost += taxes.reflectTaxesInTotalCost(
                  responseTaxes,
                  goodsList,
                  reflectAdditional
                );
              ...
              if (totalCost > 0) {
                var formatPrice = object._format.formatCost(
                  totalCost,
                  object._currency
                );
                var totalCostPanel = document.getElementById(
                  object._prefix + "totalCost"
                );
                totalCostPanel.classList.remove("hidden_panel");
                var valuePanel =
                  totalCostPanel.getElementsByClassName("value")[0];
        ...
        var rowPanel = object.createRowPanel(
          this._i18n.get("Total number of guests"),
          0,
          null,
          null,
          null
      ...
      var surchargePanel = taxes.createExtraChargesAndTaxesElement(
        object._prefix + "surchargeTaxPanel"
      );
      /**
      ...
      object._console.log("totalCost = " + totalCost);
      var formatPrice = object._format.formatCost(0, object._currency);
      var totalCostPanel = object.createRowPanel(
        object._i18n.get("Total amount"),
        formatPrice,
        "bookingFee",
        null,
    ...
    for (var i = 0; i < object._formData.length; i++) {
      ...
      if (
        targetCustomers == "visitors" &&
        object._formData[i].targetCustomers != null &&
        object._formData[i].targetCustomers == "users"
      ) {
        object._formData[i].active = "";
      } else if (
        targetCustomers == "users" &&
        object._formData[i].targetCustomers != null &&
        object._formData[i].targetCustomers == "visitors"
      ) {
        object._formData[i].active = "";
      }
      ...
      var rowPanel = object.createRowPanel(
        object._formData[i]["name"],
        value,
        object._formData[i].id,
        object._formData[i].required,
        null
      );
    ...
    if (returnButton != null) {
      returnButton.onclick = function () {
        ...
        formMainPanel.setAttribute(
          "class",
          "createFormPanel_return return_panel_for_new_booking"
        );
  ...
  this.addGuestsForRoom = function (
    guestsList,
    hotelOptions,
    input,
    totalNumberOfGuests,
    ...
    if (hotelOptions.length > 0) {
      ...
      for (var key in hotelOptions) {
        ...
        var optionsSelectPanel = input.createInput(
          hotelOptions[key]["name"],
          hotelOptions[key],
          {},
          function (event) {
            ...
            object._console.log(
              "key " + key + " index = " + index + " roomNo = " + roomNo
            );
            object._console.log(hotelOptions[key]);
            object._console.log(list);
  ...
  this.updateRoomNumber = function () {
    var object = this;
    var roomNumberLabels = document
      .getElementById("roomListPanel")
      .getElementsByClassName("roomNoLabel");
  ...
  this.lookForUser = function (callback) {
    ...
    document.getElementById("lookForUserPanel_return_button").onclick =
      function () {
    ...
    function getMembers(text) {
      ...
      new Booking_App_XMLHttp(object._url, post, object._webApp, function (
        users
      ) {
        object._loadingPanel.setAttribute("class", "hidden_panel");
  ...
  this.downloadCSV = function (
    ...
    submit.setAttribute(
      "class",
      "downloadButton button media-button button-primary button-large media-button-insert"
    );
  ...
  this.verifyForm = function (
    ...
    if (parseInt(object._calendarAccount.guestsBool) == 1) {
      if (object._calendarAccount.type == "day" && guestsList.length > 0) {
        ...
        var totalNumberOfGuestsValuePanel =
          totalNumberOfGuestsPanel.getElementsByClassName("value")[0];
        var totalNumberOfGuestsErrorPanel =
          totalNumberOfGuestsPanel.getElementsByClassName("errorMessage")[0];
        totalNumberOfGuestsValuePanel.textContent =
          responseGuests.totalNumberOfGuestsTitle;
        var responseLimitGuests = object._servicesControl.verifyToLimitGuests(
          responseGuests,
          object._calendarAccount.limitNumberOfGuests,
          object._calendarAccount.type
        );
    ...
    if (sendBool === true) {
      ...
      if (
        errorPanel != null &&
        typeof errorPanel.scrollIntoView == "function"
      ) {
  ...
  this.createRowPanel = function (name, value, id, required, actionElement) {
    ...
    if (
      ...
      value.getAttribute("data-edit") != null
    ) {
    ...
    if (
      (typeof required == "string" && required == "true") ||
      (typeof required == "number" && required == 1)
    ) {
  ...
  this.editPanelShow = function (showBool) {
    ...
    if (showBool == true) {
      ...
      if (document.getElementById("userInfoPanel") != null) {
        document
          .getElementById("userInfoPanel")
          .setAttribute("class", "hidden_panel");
      }
      ...
      if (
        object._contentPanel != null &&
        document.getElementById("showUserInfo_blockPanel") != null
      ) {
        object._contentPanel.removeChild(
          document.getElementById("showUserInfo_blockPanel")
        );
      }
  ...
  this.getServiceOrOption = function (services) {
    ...
    for (var key in services) {
      if (
        services[key].service != null &&
        parseInt(services[key].service) == 1
      ) {
        return "service";
      }
    }
  ...
  this.createServicesPanel = function (
    panel,
    services,
    optionName,
    guestsList,
        ...
        if (parseInt(service.selected) == 1) {
              ...
              for (var guestKey in guestsList) {
                ...
                if (parseInt(guest.reflectService) == 1) {
                  ...
                  if (guest.selectedName == null) {
                    guest.selectedName =
                      guest.json[parseInt(guest.number)].name;
                  }
                  //hasReflectService = true;
                  amount1 += costsWithKey[guest.costInServices] * guest.number;

                  if (defaultAmountForService == 0) {
                    defaultAmountForService =
                      costsWithKey[guest.costInServices];
                  }

                  var costInService = document.createElement("div");
                  costInService.textContent =
                    guest.name +
                    ": " +
                    object._format.formatCost(
                      costsWithKey[guest.costInServices],
                      currency
                    ) +
                    " * " +
                  ...
                  if (guest.number != null && guest.number > 0) {
                    ...
                    costInService.textContent =
                      guest.name +
                      ": " +
                      /**object._format.formatCost(costsWithKey[guest.costInServices], object._currency) + ' * ' +**/ guest.selectedName;
                    costPerGuests.appendChild(costInService);
                  }
                }
              }
          ...
          if (showAumontOnService === false) {
            if (
              ...
              if (responseCosts.hasMultipleCosts === true) {
                ...
                cost = object._i18n.get("%s to %s", [
                  object._format.formatCost(responseCosts.min, currency),
                  object._format.formatCost(responseCosts.max, currency),
                ]);
                courseCostPanel.textContent = cost;
          ...
          if (reflectService > 1) {
            ...
            courseLinePanel.onclick = function () {
              var key = parseInt(this.getAttribute("data-key"));
              var costPerGuests = document.getElementById(
                "costPerGuests_" + key
              );
              object._console.log(costPerGuests);
              object._console.log(
                costPerGuests.classList.contains("hidden_panel")
              );
          ...
          if (options.length > 0) {
            ...
            for (var i = 0; i < options.length; i++) {
              ...
              if (parseInt(option.selected) == 1) {
                ...
                if (responseCosts.hasMultipleCosts === true) {
                  //cost = object._format.formatCost(responseCosts.min, object._currency) + ' ' + object._i18n.get('to') + ' ' + object._format.formatCost(responseCosts.max, object._currency);
                  optionPricePanel.textContent = object._i18n.get("%s to %s", [
                    object._format.formatCost(
                      responseCosts.min,
                      object._currency
                    ),
                ...
                if (reflectService > 1) {
                    ...
                    for (var guestKey in guestsList) {
                      ...
                      if (parseInt(guest.reflectService) == 1) {
                        ...
                        if (defaultAmountForOption == 0) {
                          defaultAmountForOption =
                            costsWithKey[guest.costInServices];
                        }

                        amount1 +=
                          costsWithKey[guest.costInServices] * guest.number;
                        var costInOption = document.createElement("div");
                        costInOption.textContent =
                          guest.name +
                          ": " +
                          object._format.formatCost(
                            costsWithKey[guest.costInServices],
                  ...
                  optionPanel.onclick = function () {
                    ...
                    var costPerOptions = document.getElementById(
                      "costPerOptions_" + serviceKey + "_" + optionKey
                    );
                    if (
                      costPerOptions.classList.contains("hidden_panel") === true
                    ) {
                ...
                if (showAumontOnService === false) {
                  if (
                    ...
                    if (responseCosts.hasMultipleCosts === true) {
                      ...
                      optionPricePanel.textContent = object._i18n.get(
                        "%s to %s",
                        [
                          object._format.formatCost(
                            responseCosts.min,
        ...
        reserveButton.addEventListener("click", function () {
            ...
            object.sendToReceivePHP = function() {
                ...
                const payload = {
                    ...
                    tracking: {
                        referrer: "管理画面",
                        link: "https://www.ike-sunshine.co.jp/wp-admin/admin.php?page=booking-package%2Findex.php",
                        bookingID: bookingID
                    },
                    input: {
...
function ensureAgeOptions() {
  ...
  if (ageSelect.options.length < 50) {
    // 誤って削られてる時も対応
...
function initResultsPanel() {
  ...
  resultsPanel.className = "searchResultsPanel";
  resultsPanel.innerHTML = `
        <div class="results-panel-header">
            <h3>検索結果</h3>
            <span id="closeResultsPanel" class="close-button">×</span>
        </div>
        <div id="resultsContainer" class="results-panel-body"></div>
    `;
  ...
  // パネルを閉じるイベントリスナー
  document
    .getElementById("closeResultsPanel")
    .addEventListener("click", function () {
      document.getElementById("searchResultsPanel").classList.remove("active");
    });

  // スタイルを追加
  addResultsPanelStyles();
}

/**
 * 結果パネル用のスタイルを追加
 */
function addResultsPanelStyles() {
  ...
  style.textContent = `
        ...
        .approvedLabel, .pendingLabel, .canceledLabel {
...
function initCustomerSearch() {
  // 予約管理画面の上部に検索フォームを追加
  var searchFormHTML =
    '<div id="customerSearchForm" class="customerSearchForm">';
  searchFormHTML += '<select id="searchType" class="select_timezone" style="display: none">';
  searchFormHTML += '<option value="all">すべてのフィールド</option>';
  searchFormHTML += '<option value="name">名前</option>'; // 名前検索オプション追加
  searchFormHTML += '<option value="email">メールアドレス</option>';
  searchFormHTML += '<option value="phone">電話番号</option>';
  searchFormHTML += '<option value="approved">承認済み</option>';
  searchFormHTML += '<option value="pending">未承認</option>';
  searchFormHTML += '<option value="canceled">キャンセル済み</option>';
  searchFormHTML += "</select>";
  searchFormHTML +=
    '<input type="text" id="searchTermInput" placeholder="検索キーワード...">';
  searchFormHTML +=
    '<button id="searchCustomersButton" class="button button-primary">検索</button>';
  searchFormHTML +=
    '<button id="clearSearchButton" class="button">クリア</button>';
  searchFormHTML += "</div>";

  // カレンダーページの上部に検索フォームを挿入
  if (document.getElementById("calendarPage")) {
    var calendarPage = document.getElementById("calendarPage");
    var parent = calendarPage.parentNode;

    // 既存の検索フォームがある場合は削除
    var existingForm = document.getElementById("customerSearchForm");
    if (existingForm) {
    ...
    tempDiv.innerHTML = searchFormHTML;
    parent.insertBefore(tempDiv.firstChild, calendarPage);
    ...
    // 検索ボタンのイベントリスナー
    document
      .getElementById("searchCustomersButton")
      .addEventListener("click", function () {
        searchCustomers(0);
      });
 ...
 * 顧客検索を実行する
 */
function searchCustomers(offset) {
  ...
  var data = {
    action: "booking_package_search_customers",
    nonce: schedule_data.nonce,
    searchTerm: searchTerm,
    searchType: searchType,
    accountKey: parseInt(schedule_data.accountKey) || 0,
    offset: offset,
    limit: 20,
  };
  ...
  jQuery.ajax({
    ...
    success: function (response) {
      if (response.status === "success") {
        displaySearchResults(response.searchResults, searchTerm, searchType);
      } else {
        document.getElementById("resultsContainer").innerHTML =
          '<div class="error">エラー: ' +
    ...
    error: function (xhr, status, error) {
      document.getElementById("resultsContainer").innerHTML =
        '<div class="error">通信エラー: ' + error + "</div>";
    },
  });
}
...
function displaySearchResults(results, searchTerm, searchType) {
  // 検索結果をグローバル変数に保存して、詳細表示で利用できるようにする
  ...
  html += '<button class="close-sidebar-button" title="閉じる"><span class="dashicons dashicons-no-alt"></span></button>';
  html += "<h4>検索結果: " + results.totalCount + " 件</h4>";
  html += "<p>検索条件: ";

  // 検索タイプの表示名
  var searchTypeLabel = "全フィールド";
  if (searchType === "name") searchTypeLabel = "名前";
  if (searchType === "email") searchTypeLabel = "メールアドレス";
  if (searchType === "phone") searchTypeLabel = "電話番号";
  if (searchType === "approved") searchTypeLabel = "承認済み";
  if (searchType === "pending") searchTypeLabel = "未承認";
  if (searchType === "canceled") searchTypeLabel = "キャンセル済み";

  html += searchTypeLabel;
  if (
    searchType !== "approved" &&
    searchType !== "pending" &&
    searchType !== "canceled"
  ) {
    html += ' / キーワード: "' + searchTerm + '"';
  }

  html += "</p></div>";

  if (results.results.length === 0) {
    html += '<p class="no-results">該当する顧客が見つかりませんでした</p>';
  } else {
    ...
    results.results.forEach(function (customer) {
      // 名前はすでに処理済みの場合はそれを使用、ない場合はpraivateDataから抽出
      var name = customer.name || "";
      var email = customer.email || "";

      // 名前やメールがない場合、praivateDataから再抽出を試みる
      if (
        (!name || !email) &&
        customer.praivateData &&
        Array.isArray(customer.praivateData)
      ) {
        customer.praivateData.forEach(function (field) {
          if (field.isName === "true" && !name) {
            name += field.value + " ";
          }
          if (field.key === "email" && !email) {
            email = field.value;
          }
        });
      }
      ...
      // ステータスラベル
      var statusClass = customer.status || "pending";
      var statusText =
        statusClass.charAt(0).toUpperCase() + statusClass.slice(1);
      var statusLabel =
        '<span class="' + statusClass + 'Label">' + statusText + "</span>";

      html += "<tr>";
      html += "<td>" + (name.trim() || "-") + "</td>";
      html += "<td>" + (email || "-") + "</td>";
      html += "<td>" + (customer.scheduledDate || "-") + "</td>";
      html += "<td>" + statusLabel + "</td>";
      html += "<td>";
      html +=
        '<button class="button viewDetailsButton" data-key="' +
        customer.key +
        '" data-account="' +
        customer.accountKey +
        '">詳細</button>';
    ...
    if (results.maxPages > 1) {
      ...
      // 次のページへのリンク
      if (currentPage < results.maxPages) {
        html +=
  ...
  // 詳細ボタンのイベントリスナーを設定
  var detailButtons =
    resultsContainer.getElementsByClassName("viewDetailsButton");
  for (var i = 0; i < detailButtons.length; i++) {
    detailButtons[i].addEventListener("click", function () {
      var key = this.getAttribute("data-key");
      var accountKey = this.getAttribute("data-account");
      
      // 顧客詳細データを取得
      fetchCustomerDetails(key, accountKey);
    });
  }

  // サイドバー閉じるボタンのイベントリスナー
  var closeButton = resultsContainer.querySelector('.close-sidebar-button');
  if (closeButton) {
  ...
  var prevButtons = resultsContainer.getElementsByClassName("prev-page");
  for (var i = 0; i < prevButtons.length; i++) {
    prevButtons[i].addEventListener("click", function () {
      var page = parseInt(this.getAttribute("data-page"));
      searchCustomers((page - 1) * 20);
    });
  }

  var nextButtons = resultsContainer.getElementsByClassName("next-page");
...
// 顧客詳細情報を取得する関数
function fetchCustomerDetails(key, accountKey) {
  // ローディング表示
  var resultsContainer = document.getElementById("resultsContainer");
  resultsContainer.innerHTML = '<div class="loading">データを読み込み中...</div>';

  // 検索結果から指定されたキーの顧客を探す
  if (window.lastSearchResults && 
      window.lastSearchResults.results && 
      Array.isArray(window.lastSearchResults.results)) {
    
    // 検索結果から顧客データを探す
    var customer = window.lastSearchResults.results.find(function(c) {
      return c.key === key && c.accountKey === accountKey;
    });
    
    if (customer) {
      // 顧客が見つかった場合、詳細表示
      displayCustomerDetails(customer, resultsContainer);
      return;
    }
  }
  
  // 検索結果からデータが見つからない場合、別の方法を試みる
  var data = {
    action: "booking_package_get_customer_details",
    nonce: schedule_data ? schedule_data.nonce : '',
    key: key,
    accountKey: accountKey
  };
  
  // データ取得を試みる
  if (typeof jQuery !== 'undefined' && schedule_data && schedule_data.url) {
    jQuery.ajax({
      type: "POST",
      url: schedule_data.url,
      data: data,
      dataType: "json",
      success: function(response) {
        if (response && response.status === "success" && response.details) {
          displayCustomerDetails(response.details, resultsContainer);
        } else {
          // エラーの場合
          resultsContainer.innerHTML = '<div class="error">顧客データの取得に失敗しました</div>';
        }
      },
      error: function() {
        resultsContainer.innerHTML = '<div class="error">サーバーとの通信に失敗しました</div>';
      }
    });
  } else {
    // 最終的なフォールバック: 最低限の情報だけでも表示
    var minimalDetails = {
      key: key,
      accountKey: accountKey,
      status: "unknown",
    ...
    displayCustomerDetails(minimalDetails, resultsContainer);
  }
}

// 顧客詳細情報を表示する関数
function displayCustomerDetails(details, container) {
  // スタイルを追加
  addCustomStyles();
  
  var html = '<div class="customer-details">';
  
  // ヘッダー部分に戻るボタンと閉じるボタンを追加
  html += '<div class="detail-header">';
  html += '<button class="back-to-results-button"><span class="dashicons dashicons-arrow-left-alt"></span> 検索結果に戻る</button>';
  html += '<h3>顧客詳細情報</h3>';
  html += '<button class="close-sidebar-button" title="閉じる"><span class="dashicons dashicons-no-alt"></span></button>';
  html += '</div>';
  ...
  // 予約ID
  html += '<tr><th>予約ID</th><td>' + details.key + '</td></tr>';
	
	console.log(details);
  
  // ステータス
  var statusClass = details.status || "pending";
  var statusText = statusClass.charAt(0).toUpperCase() + statusClass.slice(1);
  html += '<tr><th>ステータス</th><td><span class="' + statusClass + 'Label">' + statusText + '</span></td></tr>';
  
  // 予約日時
  html += '<tr><th>予約日</th><td>' + details.scheduledDate + '</td></tr>';
  html += '<tr><th>予約時間</th><td>' + details.scheduledTime + '</td></tr>';
  
  // コース名
  const options = JSON.parse(details.options)[0];
  const selected = options["selected"];
  const option = options["options"][selected];
　html += '<tr><th>サービス</th><td>' + option.name + '</td></tr>';
//   html += '<tr><th>サービス</th><td>' + details.courseName + '</td></tr>';
	console.log(option);
//   for (let i = 0; i < details.options.length; i++) {
// 	  const option = details.options[i];
// 	  html += '<tr><th>オプション</th><td>' + option.name + '</td></tr>';
//   }
  
  // 支払い方法
  html += '<tr><th>支払い方法</th><td>' + details.payMode + '</td></tr>';
  html += '</table>';
  html += '</div>';
  ...
  // praivateDataから顧客情報を抽出して表示
  if (details.praivateData && Array.isArray(details.praivateData)) {
    details.praivateData.forEach(function(field) {
        html += '<tr><th>' + field.name + '</th><td>' + field.value + '</td></tr>';
    });
  }
  ...
  // 検索結果に戻るボタンのイベントリスナー
  var backButton = container.querySelector('.back-to-results-button');
  if (backButton) {
  ...
  // サイドバー閉じるボタンのイベントリスナー
  var closeButton = container.querySelector('.close-sidebar-button');
  if (closeButton) {
    closeButton.addEventListener('click', function() {
      // サイドパネルを閉じる
      var searchResultsPanel = document.getElementById("searchResultsPanel");
...
function addCustomStyles() {
  // 既にスタイルが追加されている場合は再追加しない
  if (document.getElementById('booking-pacage-custom-styles')) {
    return;
  }
  
  var styleElement = document.createElement('style');
  styleElement.id = 'booking-pacage-custom-styles';
  ...
  var css = `
    ...
    .close-sidebar-button .dashicons {
      font-size: 16px;
      width: 16px;
      height: 16px;
    }
    ...
    .results-header .close-sidebar-button {
      position: absolute;
      top: 0;
      right: 0;
    }
    ...
    /* ステータスラベル */
    .approvedLabel, .pendingLabel, .canceledLabel, .unknownLabel {
      display: inline-block;
      padding: 3px 8px;
      border-radius: 3px;
      font-size: 12px;
  ...
  if (styleElement.styleSheet) {
    styleElement.styleSheet.cssText = css; // IE対応
  } else {
...
// ページ読み込み時に検索フォームと結果パネルを初期化
document.addEventListener("DOMContentLoaded", function () {
  if (document.getElementById("booking_pacage_booked_customers")) {
    initCustomerSearch();
